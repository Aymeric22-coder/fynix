...
export default async function DashboardPage() {
  ...
  const [assetsRes, debtsRes, lotsRes, snapshotsRes] = await Promise.all([
    supabase.from('assets').select('id,name,asset_type,current_value,acquisition_price,confidence,last_valued_at').eq('user_id', user!.id).eq('status','active'),
    supabase.from('debts').select('capital_remaining,monthly_payment').eq('user_id', user!.id).eq('status','active'),
    supabase.from('real_estate_lots').select('rent_amount,charges_amount,status').eq('user_id', user!.id),
    supabase.from('patrimony_snapshots').select('snapshot_date,total_net_value,total_gross_value,total_debt').eq('user_id', user!.id).order('snapshot_date',{ascending:false}).limit(13),
  ])
  ...
  const monthlyLoan     = debts.reduce((s, d) => s + (d.monthly_payment ?? 0), 0)
  const monthlyRents    = lots.filter(l => l.status === 'rented').reduce((s, l) => s + (l.rent_amount ?? 0), 0)
  const monthlyCharges  = lots.reduce((s, l) => s + (l.charges_amount ?? 0), 0)
  const cashFlow        = monthlyRents - monthlyCharges - monthlyLoan

  // CAGR
  let cagrValue: number | null = null
  if (snapshots.length >= 2) {
    const latest = snapshots[0]!; const oldest = snapshots[snapshots.length - 1]!
  ...
  for (const { type, percent } of donutData)
    if (percent > 70) alerts.push({ type: 'over_exposure', message: `Sur-exposition ${ASSET_TYPE_LABELS[type] ?? type} : ${percent.toFixed(0)} % du patrimoine`, severity: 'warning' })

  const thirtyDaysAgo = new Date(); thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30)
  const stale = assets.filter(a => a.last_valued_at && new Date(a.last_valued_at) < thirtyDaysAgo)
  if (stale.length) alerts.push({ type: 'stale_data', message: `${stale.length} actif(s) non valorisé(s) depuis +30 jours`, severity: 'info' })
