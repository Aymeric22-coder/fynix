 ...
 * `revenu_passif_cible` de StepRisqueFire.
 *
 * Vérifie :
...
import { StepRisqueFire } from '../StepRisqueFire'
...
type AnySet = any   // pour préserver la généricité du `set` de StepRisqueFire dans les tests

/** Rerender helper : StepRisqueFire lit `values` par référence, donc on remonte le
 *  composant à chaque mutation du store pour simuler le parent qui reflow. */
function renderStepRisqueFire(store: QuestionnaireValues, set: AnySet) {
  return render(<StepRisqueFire values={store} set={set} />)
}
