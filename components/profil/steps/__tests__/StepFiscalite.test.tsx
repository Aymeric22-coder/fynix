...
describe('StepFiscalite — Ta fiscalité (CS1, ID 4 post-renumérotation)', () => {
  it('rend les 6 chips attendues', () => {
    const { store, set } = harness()
    render(<StepFiscalite values={store} set={set} />)
  ...
  it('clic sur la chip 41 % écrit tmi_rate = 41', () => {
    const { set } = harness()
    render(<StepFiscalite values={EMPTY_VALUES} set={set} />)
  ...
  it('clic sur « Je ne sais pas » écrit tmi_rate = null', () => {
    ...
    render(<StepFiscalite values={{ ...EMPTY_VALUES, tmi_rate: 30 }} set={set} />)
  ...
  it('tmi_rate=null → « Je ne sais pas » est aria-pressed=true + hint affiché', () => {
    ...
    render(<StepFiscalite values={{ ...EMPTY_VALUES, tmi_rate: null }} set={set} />)
