...
Date initiale : 2026-05-21 · Dernière mise à jour : 2026-05-27 (V16)
...
| **V11** | **CAS-DASH-001 / INTEG-003** alertes impayé. Helper pur `lib/real-estate/property-alerts.ts` : `computeUnpaidRentAlerts(events, today)` agrège les `property_events.kind='rent_unpaid' AND is_resolved=false` par bien, retourne `count` + `totalUnpaidEur` (positif) + `daysSinceOldest` + `severity` progressive (info < 30 j, warning ≥ 30 j ou ≥ 2 events, critical > 60 j ou ≥ 3 events). Branché dans `app/(app)/immobilier/page.tsx` via une query batchée ; `alertCount` désormais alimenté (avant : 0 hardcodé). `PropertySummary.unpaidRent?` + `PortfolioAlert{ kind:'unpaid_rent' }` propagé jusqu'au bandeau. Bloc `under_rent` historique désactivé (faux émetteur qui se déclenchait sur tout `alertCount > 0` — sera rebranché avec vrai signal marché). 25 nouveaux tests | ✅ Mergé direct master | — |
| **V12** | **CAS-RP-001 / CAS-WIZ-LOT-001** résidence principale / secondaire. Helper pur `lib/real-estate/validate-wizard.ts` : `isRentalWizardUsage`, `wizardStepsFor` (5 étapes locatif vs 4 étapes RP/RS), `requiresFiscalRegimeStep`. **Fiche** : 4 onglets visibles pour `!isRental` (Synthèse, Crédit, Amortissement, Charges) — Rentabilité, Suivi réel, Dispositif fiscal masqués. Carte 5 Synthèse remplace "Type d'usage" par **"Capital remboursé"** (= principal initial − CRD) + % progression. Fallback Tabs automatique sur 1er onglet si `?tab=` pointe vers un onglet masqué. **Wizard** : STEPS dynamique selon `usage_type`, étape 4 devient « Récapitulatif » pour RP/RS (sans champ), `validateStep(4)` n'exige pas de régime, POST `fiscal_regime: null` + pas de POST /lots forcé. `useEffect` clamp `step` si `usage_type` change de locatif → RP en mid-flow. 17 nouveaux tests sur le helper | ✅ Mergé direct master | — |
| **V13** | **MH option fantôme + garde-fou Pinel fermé.** (a) `monuments_historiques` et `malraux` marqués `disabled` dans le sélecteur d'`incentive-form.tsx` avec libellé « (non pris en charge pour l'instant) » — les options restent dans la liste pour qu'un bien déjà sauvegardé s'affiche proprement, mais ne sont plus sélectionnables. (b) Nouveau `isPinelClosedForAcquisition(kind, acquisitionDate)` + constante `PINEL_CLOSING_DATE = '2024-12-31'` dans `reduction-schedule.ts`. `buildIncentiveReductionPerYear` accepte un `acquisitionDate?` optionnel et retourne un tableau de zéros pour Pinel/Pinel+ acquis strictement après cette date (Denormandie non concerné — prolongé jusqu'au 31/12/2027). UI : bandeau `text-danger bg-danger-muted` dans `IncentiveTabContent` qui masque le panel et affiche le message « Le Pinel est fermé aux nouveaux investissements depuis le 31/12/2024 — non applicable à un bien acquis après cette date ». `acquisition_date` passée depuis `page.tsx` (déjà chargée via asset). 17 nouveaux tests | ✅ Mergé direct master | — |
| **V14** | **Export PDF multi-crédit** (annexe dette technique). Avant : la route `export-pdf/route.ts` lisait `.maybeSingle()` sur `debts` → le PDF d'un bien à 2 crédits ignorait le PTZ ; mensualité, CRD et cash-flow divergeaient de la fiche. Désormais : SELECT TOUS les `debts` actifs, tri `principal`-first, propagation en `DbDebt[]` à `buildSimulationInputFromDb` (signature multi-crédit V3.1 enfin honorée par le PDF). Apport personnel corrigé en `acqCost − sum(initial_amount)` (avant : sous-évaluait l'apport en omettant le PTZ). `AnnualReportInput.debts?: DbDebt[]` ajouté (optionnel, fallback `input.debt.initial_amount` pour la rétrocompat tests historiques mono-crédit). Pied de page 2 ajoute une mention « Bien à N crédits actifs — détails sur le principal, mensualités du tableau = agrégat » quand `debts.length > 1`. 6 nouveaux tests : 3 invariants (kpis simulation PDF == fiche, multi > mono pour mensualité+CRD, rétrocompat mono sans `debts`), 2 cas route (mock supabase 2 debts, ordre PTZ-avant-principal en DB) | ✅ Mergé direct master | — |
| **V15** | **Lot lisibilité + mobile** (3 frictions UX). (a) **FRICTION-003** tableau Rentabilité : préfixe `-` ajouté aux colonnes Charges et Crédit de `simulation-panel.tsx` (avant : `text-danger` rouge mais aucun signe → lecture ambiguë). Pattern aligné sur la colonne Vacance existante : `value > 0 ? \`-${formatCurrency(...)}\` : '—'`. (b) **FRICTION-005** PropertyCard : passage de `grid-cols-3` fixe à `grid-cols-2 sm:grid-cols-3` sur les deux grilles (ligne valeur/CRD/valeur nette + ligne cash-flow/rdt/PV). Plus de saturation sous ~380 px. (c) **FRICTION-008** historique charges : wrapper `<div className="overflow-x-auto">` autour de la `<table>` 6 colonnes dans `app/(app)/immobilier/[id]/page.tsx` — défilement horizontal propre sur mobile. Aucun nouveau test (changements purement présentationnels, design system existant) | ✅ Mergé direct master | — |
| **V16** | **Finition CCA SCI à l'IS** (saisie + plafond cash). Avant : `cca_amount` (mig 037) écrit nulle part dans l'app — colonne toujours à 0, option « Remboursement CCA » jamais visible. (a) **Saisie** : champ Money « Compte courant d'associé » ajouté dans `EditPropertyPanel > FiscalSection` et dans le wizard `nouveau/page.tsx` étape Régime fiscal, conditionnel `fiscal_regime === 'sci_is'`. Routes API enrichies : `POST /api/real-estate` insère `cca_amount` (default 0), `PATCH /api/real-estate/[id]` accepte le champ, validation `cca_amount: number ≥ 0` côté serveur sur les deux routes (rejette `-1`, types non numériques, accepte `null` pour effacement). (b) **Plafond cash** : `computeDividendDistribution` dans `lib/real-estate/fiscal/sci-is.ts` plafonne désormais `ccaReimbursement = min(ccaAvailable, max(0, availableCashYear))` au lieu de `min(ccaAvailable, netProfitAfterIS)`. Le bénéfice comptable n'est PAS la bonne contrainte : une SCI fortement amortie peut avoir `netProfitAfterIS ≈ 0` tout en dégageant un cash positif qui rembourse l'associé. Nouvelle prop `availableCashYear: number` sur `SciDistribution` ; `page.tsx` passe `simResult.projection[0].cashFlowAfterTax`. Légende UI mise à jour : « plafonné à la trésorerie » (au lieu de « plafonné au profit »). 5 tests PATCH (cca_amount valide, =0, <0 rejet, type string rejet, null OK) + refonte des 6 tests `sci-dividends.test.ts` avec le nouveau plafond + 3 cas inédits (SCI amortie bénéfice 0 cash positif → CCA remboursable, cash négatif → CCA bloqué à 0, CCA négatif sécurisé à 0). 14 nouveaux tests | ✅ Mergé direct master | — |
| **V17+** | Cf. plan section 8 — non démarrées (BUG-005 Pinel éligibilité [bloqué P7 surface en DB], INTEG-007 réversibilité révision loyer [décision produit], autres kinds alerte vacancy / sinistre / credit_ending_soon / under_rent, décisions produit foncier MaPrimeRénov / LMP déficit amort / agrégation foyer / MH/Malraux à brancher au moteur, 6 frictions UX P3 restantes) | ⏳ À venir | — |

**Total items traités à ce jour** : **35/41** (P1: 11/11 — ROB-001/002/003 + BUG-002/003 + INTEG-001/002 + BUG-007/008 + BUG-001 + BUG-004 + BUG-006 + CAS-DASH-001/INTEG-003 + CAS-RP-001 ; P2: 18/19 — régen types + BUG-009 + INTEG-005/006 + BUG-D1-M01 + BUG-D1-M02 + BUG-D1-M08 + BUG-D1-M03 + BUG-D1-M04 + BUG-D1-M05 + BUG-D1-M06 + ROB-101/102/103/104/105/106/107 + CAS-WIZ-LOT-001 ; FRICTION: 4/10 — FRICTION-001 + FRICTION-003 + FRICTION-005 + FRICTION-008 ; PDF: 1/1 — annexe dette technique V14) + **3 finitions hors plan** : V13 MH option fantôme désactivée + garde-fou Pinel clos 2025, V16 CCA SCI IS (saisie UI + plafond cash) · **≥175 nouveaux tests** ajoutés (V1–V14 + V16 = 14 ; V15 = 0 car changements purement présentationnels) — 1548 tests totaux verts.
...
| 6 | ✅ **CAS-DASH-001 / INTEG-003** — Helper `computeUnpaidRentAlerts` + query batchée `property_events` filtré, `alertCount` alimenté, push `PortfolioAlert{ kind:'unpaid_rent' }` jusqu'au bandeau. Bloc `under_rent` historique désactivé (faux émetteur) _(V11)_ | M | Intégration majeure manquante |
...
| 33 | ✅ **FRICTION-003** — Préfixe `-` ajouté sur les colonnes Charges et Crédit du tableau Rentabilité (aligné sur le pattern existant de la colonne Vacance) _(V15)_ | S |
| 34 | **FRICTION-004** — `ChargesWarningBanner` dans l'onglet Charges si `initial == null` | S |
| 35 | ✅ **FRICTION-005** — `grid-cols-2 sm:grid-cols-3` appliqué sur les 2 grilles de PropertyCard (ligne valeur/CRD/valeur nette + ligne cash-flow/rdt/PV) _(V15)_ | S |
| 36 | **FRICTION-006** — Tooltip TAEG + palette neutre | S |
| 37 | **FRICTION-007** — Remplacer emojis 🔴🟡🔵 par `<Badge variant="…">` | S |
| 38 | ✅ **FRICTION-008** — `<div className="overflow-x-auto">` autour du tableau historique charges (6 colonnes) sur la page bien _(V15)_ | S |
...
- ✅ **PDF export** ([app/api/real-estate/[id]/export-pdf/route.ts](../app/api/real-estate/[id]/export-pdf/route.ts)) — SELECT multi-debts + propagation `DbDebt[]` à `buildSimulationInputFromDb` + apport sur somme des principals + invariant testé (PDF == fiche pour bien à 2 crédits) _(V14)_
